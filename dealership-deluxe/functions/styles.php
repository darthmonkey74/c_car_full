...
function theme_options_do_page() {
	...
	<div class="wrap">
		...
		<form method="post" action="options.php">
			<?php 
				settings_fields( 'dealership_options' );
				$options = get_option( 'dealership_theme_options' ); 
			...
			<table class="form-table">
              <hr />
              <h2><?php _e('Dealership Color Selection','language')?></h2>
                <tr valign="top">
					<td>
                        <label class="description" for="dealership_theme_options[color]"><?php _e('Select a color style for your website: ','language')?></label>
						<select id="dealership_theme_options[color]" class="regular-text" name="dealership_theme_options[color]" style="width:150px; color: #333;">
						<option value="style" <?php echo ($theme_color == "style") ? "selected='selected'" : ''; ?>><?php _e('Red','language')?></option>
						...
						<option value="blue" <?php echo ($theme_color == "blue") ? "selected='selected'" : ''; ?>><?php _e('Blue','language')?></option>
						</select>
					</td>
				</tr>
			</table>
